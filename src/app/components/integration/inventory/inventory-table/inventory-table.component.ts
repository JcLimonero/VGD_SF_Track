...
@Component({
  selector: 'integration-inventory-table',
  standalone: true,
  imports: [CommonModule, GenericTableComponent],
  templateUrl: './inventory-table.component.html',
  styleUrl: './inventory-table.component.scss'
})
export class IntegrationInventoryTableComponent implements OnInit {
  data: any[] = [];
  loading = false;
  ...
  loadPage(pageIndex: number, pageSize: number): void {
    ...
    this.vanguardiaApi.getIntegrationHondaInventory(params).subscribe({
      next: (res) => {
        this.data = res.items || [];
  ...
  downloadExcel(): void {
    ...
    for (let page = 1; page <= totalPages; page++) {
      pageRequests.push(this.vanguardiaApi.getIntegrationHondaInventory({ ...baseParams, page }));
    }
    ...
    forkJoin(pageRequests).subscribe({
      next: (responses) => {
        try {
          ...
          XLSX.utils.book_append_sheet(workbook, worksheet, 'IntegrationInventario');
          ...
          XLSX.writeFile(workbook, `integration_inventario_${allData.length}_${timestamp}.xlsx`);
        } finally {
